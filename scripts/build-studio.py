...
'es': {
 ...
 'nav':['Soluciones','Sobre mí','Hablemos'], 'skip':'Ir al contenido', 'menu':'Menú',
...
'en': {
 ...
 'nav':['Solutions','About','Let’s talk'], 'skip':'Skip to content', 'menu':'Menu',
...
SHOWCASE = {
    'es': {
        ...
        'index_note': 'Cuatro casos detallados para mantenimiento, reservas, administración y presupuestos de obra.',
    ...
    'en': {
        ...
        'index_note': 'Four detailed cases across maintenance, bookings, administration and trade quotations.',
...
CONTRACTOR = {
    'es': {
        'label': 'Caso ficticio', 'name': 'Cerrajería Faro', 'eyebrow': 'IA para visitas y presupuestos',
        'title': 'De una visita a un presupuesto listo para enviar.',
        'intro': 'Una herramienta que organiza fotos y medidas, consulta el catálogo de materiales, prepara partidas y calcula el IGIC antes de que el profesional apruebe el precio.',
        'short': 'Fotos, medidas, materiales y mano de obra convertidos en un presupuesto claro y revisable.',
        'explore': 'Explorar el caso', 'how': 'Del trabajo observado<br>al precio explicado.',
        'how_intro': 'La IA ayuda a estructurar la visita y encontrar referencias. El cerrajero confirma el diagnóstico, las cantidades, el margen y el precio final.',
        'product_title': 'Catálogo, trabajo e impuestos,<br>en un solo presupuesto.',
        'product_intro': 'Una mesa de trabajo para construir la oferta y una vista móvil para revisarla con el cliente antes de enviarla.',
        'before': 'Antes', 'before_text': 'Fotos en el móvil, precios buscados en varios catálogos y cálculos rehechos para cada presupuesto.',
        'after': 'Con Cerrajería Faro', 'after_text': 'La visita conserva sus medidas, materiales sugeridos, horas previstas, impuestos y decisiones del profesional.',
        'result_items': ['Cada partida muestra de dónde sale', 'El catálogo mantiene materiales y precios conectados', 'El profesional confirma alcance, margen e impuestos'],
        'stages': [('visit','01','Visita registrada','Fotos, medidas y nota de voz quedan juntas.'),('assessed','02','Trabajo propuesto','La IA estructura el trabajo observado.'),('catalog','03','Materiales encontrados','El catálogo ofrece referencias compatibles.'),('labour','04','Mano de obra calculada','Horas y desplazamiento se añaden como partidas.'),('reviewed','05','Presupuesto revisado','El profesional ajusta cantidades y margen.'),('quoted','06','Oferta preparada','Base, IGIC y total quedan listos para enviar.')]
    },
    'en': {
        'label': 'Fictional case', 'name': 'Cerrajería Faro', 'eyebrow': 'AI for site visits and quotes',
        'title': 'From a site visit to a quote ready to send.',
        'intro': 'A tool that organises photos and measurements, searches the materials catalogue, prepares line items and calculates IGIC before the tradesperson approves the price.',
        'short': 'Photos, measurements, materials and labour turned into a clear, reviewable quote.',
        'explore': 'Explore the case', 'how': 'From observed work<br>to an explained price.',
        'how_intro': 'AI helps structure the visit and find references. The locksmith confirms the assessment, quantities, margin and final price.',
        'product_title': 'Catalogue, work and tax,<br>in one quote.',
        'product_intro': 'A workspace for building the offer and a mobile view for reviewing it with the customer before sending.',
        'before': 'Before', 'before_text': 'Photos on a phone, prices searched across catalogues and calculations rebuilt for every quote.',
        'after': 'With Cerrajería Faro', 'after_text': 'The visit keeps its measurements, suggested materials, estimated hours, tax and the tradesperson’s decisions.',
        'result_items': ['Every line item shows where it comes from', 'The catalogue keeps materials and prices connected', 'The tradesperson confirms scope, margin and tax'],
        'stages': [('visit','01','Visit recorded','Photos, measurements and voice note stay together.'),('assessed','02','Work proposed','AI structures the observed work.'),('catalog','03','Materials found','The catalogue offers compatible references.'),('labour','04','Labour calculated','Hours and travel become line items.'),('reviewed','05','Quote reviewed','The tradesperson adjusts quantities and margin.'),('quoted','06','Offer prepared','Subtotal, IGIC and total are ready to send.')]
    },
}


def shared_head(lang, title, description, canonical, alternate, image='/assets/showcase/nexo/field-service.webp'):
    return f'''<!doctype html><html lang="{lang}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{escape(title)} — AlexisSantos.dev</title><meta name="description" content="{escape(description, quote=True)}"><link rel="canonical" href="https://alexissantos.dev{canonical}"><link rel="alternate" hreflang="es" href="https://alexissantos.dev{canonical if lang == 'es' else alternate}"><link rel="alternate" hreflang="en" href="https://alexissantos.dev{alternate if lang == 'es' else canonical}"><link rel="alternate" hreflang="x-default" href="https://alexissantos.dev{canonical if lang == 'es' else alternate}"><meta property="og:site_name" content="AlexisSantos.dev"><meta property="og:locale" content="{'es_ES' if lang == 'es' else 'en_GB'}"><meta property="og:title" content="{escape(title, quote=True)}"><meta property="og:description" content="{escape(description, quote=True)}"><meta property="og:type" content="website"><meta property="og:url" content="https://alexissantos.dev{canonical}"><meta property="og:image" content="https://alexissantos.dev{image}"><meta name="twitter:card" content="summary_large_image"><meta name="theme-color" content="#f5f6f8"><link rel="icon" href="/assets/studio/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="/assets/studio/studio.css"><link rel="stylesheet" href="/assets/showcase/showcase.css"><script src="/assets/studio/studio.js" defer></script><script src="/assets/showcase/showcase.js" defer></script></head>'''
...
def contractor_layers(lang):
    es = lang == 'es'
    return f'''
    <div class="stage-layer" data-stage-panel="visit"><div class="quote-card"><span class="ui-kicker">{'VISITA · LA CALETA' if es else 'VISIT · LA CALETA'}</span><h3>{'Puerta de entrada desalineada' if es else 'Misaligned entrance door'}</h3><p>{'3 fotos · 4 medidas · nota de voz' if es else '3 photos · 4 measurements · voice note'}</p></div></div>
    <div class="stage-layer" data-stage-panel="assessed"><div class="quote-card"><span class="ui-kicker">{'PROPUESTA DE TRABAJO' if es else 'PROPOSED WORK'}</span><h3>{'Sustituir bombín y ajustar marco' if es else 'Replace cylinder and adjust frame'}</h3><ul><li>✓ {'Bombín de seguridad 35/35' if es else '35/35 security cylinder'}</li><li>✓ {'Ajuste de cerradero' if es else 'Strike plate adjustment'}</li><li>✓ {'Prueba de cierre' if es else 'Closing test'}</li></ul></div></div>
    <div class="stage-layer" data-stage-panel="catalog"><div class="quote-card catalog-picks"><span class="ui-kicker">{'CATÁLOGO DE MATERIALES' if es else 'MATERIALS CATALOGUE'}</span><div><span>CF-3535<b>{'Bombín seguridad' if es else 'Security cylinder'}</b><i>86,00 €</i></span><span>PL-08<b>{'Placa reforzada' if es else 'Reinforced plate'}</b><i>34,00 €</i></span></div></div></div>
    <div class="stage-layer" data-stage-panel="labour"><div class="quote-card"><span class="ui-kicker">{'MANO DE OBRA' if es else 'LABOUR'}</span><h3>2,5 h · 112,50 €</h3><p>{'Instalación, ajuste, desplazamiento y prueba final.' if es else 'Installation, adjustment, travel and final test.'}</p></div></div>
    <div class="stage-layer" data-stage-panel="reviewed"><div class="quote-card quote-review"><span class="ui-kicker">{'REVISIÓN DEL PROFESIONAL' if es else 'TRADESPERSON REVIEW'}</span><h3>{'Alcance y margen confirmados' if es else 'Scope and margin confirmed'}</h3><p>{'Carlos ajustó la mano de obra y aprobó las referencias.' if es else 'Carlos adjusted labour and approved the references.'}</p><button>{'Aprobar presupuesto' if es else 'Approve quote'} →</button></div></div>
    <div class="stage-layer" data-stage-panel="quoted"><div class="quote-card quote-total"><span class="ui-kicker">{'PRESUPUESTO #CF-204' if es else 'QUOTE #CF-204'}</span><dl><div><dt>{'Base' if es else 'Subtotal'}</dt><dd>312,50 €</dd></div><div><dt>IGIC · 7%</dt><dd>21,88 €</dd></div><div><dt>Total</dt><dd>334,38 €</dd></div></dl><button>{'Listo para enviar' if es else 'Ready to send'} →</button></div></div>'''


def contractor_mosaic(lang, context='case'):
    s = CONTRACTOR[lang]
    controls = ''.join(f'''<button class="stage-control" type="button" data-stage="{stage}" data-status="{escape(title, quote=True)} — {escape(description, quote=True)}"><span class="stage-number">{number}</span><span><b>{title}</b><small>{description}</small></span><span class="stage-arrow" aria-hidden="true">↗</span></button>''' for stage, number, title, description in s['stages'])
    return f'''<div class="showcase-mosaic contractor-mosaic" data-showcase data-active-stage="visit"><div class="showcase-panel"><img src="/assets/showcase/cerrajeria-faro/door-assessment.webp" alt="" width="1536" height="1024" loading="{'eager' if context == 'case' else 'lazy'}"><div class="panel-shade"></div>{contractor_layers(lang)}<p class="showcase-status" aria-live="polite">{s['stages'][0][2]} — {s['stages'][0][3]}</p></div><div class="showcase-stage-list">{controls}</div></div>'''


def contractor_product_mockup(lang):
    s = CONTRACTOR[lang]; es = lang == 'es'
    return f'''<section class="product-showcase contractor-product"><div class="wrap product-showcase-intro"><div><span class="showcase-kicker">{'PRODUCTO' if es else 'PRODUCT'}</span><h2>{s['product_title']}</h2></div><p>{s['product_intro']}</p></div><div class="wrap quote-devices"><div class="quote-desktop"><div class="desktop-bar"><span></span><span></span><span></span><b>CERRAJERÍA FARO / {'PRESUPUESTOS' if es else 'QUOTES'}</b></div><div class="quote-screen"><aside><strong>FARO</strong><nav><span class="active">{'Presupuesto' if es else 'Quote'}</span><span>{'Catálogo' if es else 'Catalogue'}</span><span>{'Clientes' if es else 'Customers'}</span></nav></aside><main><header><small>#CF-204 · LA CALETA</small><h3>{'Puerta de entrada' if es else 'Entrance door'}</h3></header><div class="quote-builder"><section><h4>{'Partidas' if es else 'Line items'}</h4><div><b>{'Bombín de seguridad 35/35' if es else '35/35 security cylinder'}</b><span>86,00 €</span></div><div><b>{'Placa reforzada' if es else 'Reinforced plate'}</b><span>34,00 €</span></div><div><b>{'Mano de obra · 2,5 h' if es else 'Labour · 2.5 h'}</b><span>112,50 €</span></div><div><b>{'Desplazamiento y ajuste' if es else 'Travel and adjustment'}</b><span>80,00 €</span></div></section><aside><span>{'RESUMEN' if es else 'SUMMARY'}</span><dl><div><dt>{'Base' if es else 'Subtotal'}</dt><dd>312,50 €</dd></div><div><dt>IGIC 7%</dt><dd>21,88 €</dd></div><div><dt>Total</dt><dd>334,38 €</dd></div></dl><button>{'Aprobar y preparar PDF' if es else 'Approve and prepare PDF'} →</button></aside></div></main></div></div><div class="quote-phone"><div class="phone-speaker"></div><div class="quote-phone-screen"><header><span>9:41</span><b>FARO</b><span>•••</span></header><small>{'PRESUPUESTO PREPARADO' if es else 'QUOTE PREPARED'}</small><h3>{'Puerta de entrada' if es else 'Entrance door'}</h3><p>La Caleta · #CF-204</p><b>334,38 €</b><dl><div><dt>{'Base' if es else 'Subtotal'}</dt><dd>312,50 €</dd></div><div><dt>IGIC 7%</dt><dd>21,88 €</dd></div></dl><button>{'Enviar al cliente' if es else 'Send to customer'} →</button></div></div></div></section>'''
...
def build_showcase_page(lang):
    ...
    contractor_url = '/showcase/cerrajeria-faro/' if es else '/en/showcase/cerrajeria-faro/'
    restaurant = RESTAURANT[lang]
    invoice = INVOICES[lang]
    contractor = CONTRACTOR[lang]
...
def build_contractor_page(lang):
    s = CONTRACTOR[lang]; es = lang == 'es'
    canonical = '/showcase/cerrajeria-faro/' if es else '/en/showcase/cerrajeria-faro/'
    alternate = '/en/showcase/cerrajeria-faro/' if es else '/showcase/cerrajeria-faro/'
    index_url = '/showcase/' if es else '/en/showcase/'; contact_url = '/#contact' if es else '/en/#contact'
    header, footer = showcase_shell_parts(lang, '/showcase/cerrajeria-faro/', '/en/showcase/cerrajeria-faro/')
    head = shared_head(lang, s['name'], s['intro'], canonical, alternate, '/assets/showcase/cerrajeria-faro/door-assessment.webp')
    results = ''.join(f'<li><span>0{i}</span>{item}</li>' for i, item in enumerate(s['result_items'], 1))
    return f'''{head}<body id="top"><a class="skip" href="#main">{COPY[lang]['skip']}</a>{header}<main id="main" class="showcase-page contractor-case"><section class="case-hero wrap"><a class="case-back" href="{index_url}">← {'Todas las demostraciones' if es else 'All showcase cases'}</a><div class="case-heading"><div><span class="fictional-label">{s['label']}</span><span>{s['eyebrow']}</span></div><h1>{s['name']}</h1><p>{s['title']} {s['intro']}</p></div></section><section class="wrap case-demo">{contractor_mosaic(lang, 'case')}</section><section class="case-explainer wrap"><div><span class="showcase-kicker">{'EL FLUJO' if es else 'THE WORKFLOW'}</span><h2>{s['how']}</h2></div><p>{s['how_intro']}</p></section>{contractor_product_mockup(lang)}<section class="before-after wrap"><article><span>01 / {s['before'].upper()}</span><h3>{s['before']}</h3><p>{s['before_text']}</p></article><article><span>02 / {s['after'].upper()}</span><h3>{s['after']}</h3><p>{s['after_text']}</p></article></section><section class="case-results"><div class="wrap"><div><span class="showcase-kicker">{'RESULTADO' if es else 'OUTCOME'}</span><h2>{'Lo que cambia' if es else 'What changes'}.</h2></div><ol>{results}</ol></div></section><section class="case-cta wrap"><h2>{'¿Preparar presupuestos te roba las tardes?' if es else 'Do quotes consume your evenings?'}</h2><div><p>{'Conecto la visita, tu catálogo y tus reglas de precio sin quitarte la decisión final.' if es else 'I connect the site visit, your catalogue and pricing rules while keeping the final decision with you.'}</p><a class="button" href="{contact_url}">{'Cuéntame cómo presupuestas' if es else 'Tell me how you quote'} {ARROW}</a></div></section></main>{footer}</body></html>'''
for lang,t in COPY.items():
 url='/' if lang=='es' else '/en/'
 locale,alt_locale=('es_ES','en_GB') if lang=='es' else ('en_GB','es_ES')
 share=f'https://alexissantos.dev/assets/studio/og-{lang}.jpg'
...
<meta property="og:site_name" content="AlexisSantos.dev"><meta property="og:locale" content="{locale}"><meta property="og:locale:alternate" content="{alt_locale}"><meta property="og:title" content="{t['title']}"><meta property="og:description" content="{t['description']}"><meta property="og:type" content="website"><meta property="og:url" content="https://alexissantos.dev{url}"><meta property="og:image" content="{share}"><meta property="og:image:type" content="image/jpeg"><meta property="og:image:width" content="1200"><meta property="og:image:height" content="630"><meta property="og:image:alt" content="AlexisSantos.dev — {t['footer']}"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="{t['title']}"><meta name="twitter:description" content="{t['description']}"><meta name="twitter:image" content="{share}"><meta name="twitter:image:alt" content="AlexisSantos.dev — {t['footer']}"><meta name="twitter:creator" content="@deepfirstsearch"><meta name="theme-color" content="#f5f6f8">
...
for lang in ['es', 'en']:
    ...
    contractor_path = ROOT / ('showcase/cerrajeria-faro/index.html' if lang == 'es' else 'en/showcase/cerrajeria-faro/index.html')
    ...
    contractor_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    contractor_path.write_text(build_contractor_page(lang))
